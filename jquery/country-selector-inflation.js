...
$(function() {
	$('.inflation-button').not('.show-all-button').click(function() {
		...
		// Reveal the single global "What's next?" block at the bottom of the page
		$('#global-whats-next').show();

		// Re-populate the dynamic stat cards for the selected currency
		if (typeof window.loadInflationStats === 'function') {
			window.loadInflationStats(selectedCurrency);
		}
	...
	$('.show-all-button').click(function() {
		...
		if ($('.show-all-button .choose-back').is(':visible')) {
			...
			// Hide the global "What's next?" block again — nothing is selected
			$('#global-whats-next').hide();
