...
- [x] **Add "Reviewed for accuracy" badge to content pages** — Instead of showing "Last Updated" dates (which can backfire on evergreen content by making it look stale when months pass between updates), add a visible "Reviewed for accuracy: [year]" line to each educational page. This signals editorial rigor without implying content is outdated. Use year-only format (e.g., "2026") which ages more slowly than specific dates. Can be refreshed with an annual content review pass. Natural placement: inside the existing publisher-attribution block. The `dateModified` in Article JSON-LD schemas continues to track actual content changes separately for machines. **Why not "Last Updated"?** Pages with old visible dates are a *negative* signal — a page with no date is better than a page saying "Last Updated: 14 months ago." The "Reviewed for accuracy" pattern (used by medical/financial sites) avoids this trap while still demonstrating editorial oversight. ✅ *Completed Mar 28, 2026 — Added "✓ Reviewed for accuracy: 2026" line to all 33 publisher-attribution blocks via `scripts/inject-reviewed-badge.js`. Badge appears as the last line inside the existing `<p>` tag with `<span class="reviewed-badge" data-i18n="common_reviewed_accuracy">`. Added `common_reviewed_accuracy` translation key to all 10 language JSON files (en, de, es, fr, pt, nl, bg, id, th, it) with actual translations via `scripts/add-reviewed-accuracy-key.js`. CSS: `.reviewed-badge` with subtle green (#6c9) text, 13px font, slight letter-spacing. Script is idempotent.*

- [x] **Add source citations inline** — The inflation page mentions statistics like "Nearly 80% of all US Dollars that exist today were created between 2020 and 2023" with a link to FRED. Formalize these as citations: *"According to the Federal Reserve Economic Data (FRED)..."* AI engines are more likely to cite content that itself cites authoritative sources. ✅ *Completed Mar 28, 2026 — Added inline source citation links to 12 data-heavy pages. Inflation page: 14 FRED links added to currency-specific money supply claims (USD already had one, now all 15 currencies have FRED links). Bank-runs: SVB collapse linked to FDIC press release. Bitcoin-vs-gold: World Gold Council citation for 1.6% annual supply growth. Bitcoin-vs-cash: RBI demonetization notice citation. Bitcoin-vs-visa: Visa interchange fee schedule + Federal Reserve credit card rate data. Bitcoin-vs-bonds: Treasury auction results citation. Bitcoin-vs-crypto: Bitcoin whitepaper citation for Proof of Work. Scripts: `scripts/add-inline-citations.js` and `scripts/add-inline-citations-v2.js` (both idempotent).*

- [x] **Add a references/sources section to data-heavy pages** — At the bottom of pages like `/inflation`, `/bitcoin-vs-gold`, `/bitcoin-vs-stocks`, add a "Sources" section listing the external references used. This dramatically increases AI trust in the content. ✅ *Completed Mar 28, 2026 — Added "Sources" reference sections to all 12 data-heavy pages via `scripts/inject-sources-section.js`: inflation (6 sources: FRED, BLS, mempool.space, Bitcoin source code, whitepaper), bank-runs (5 sources: FDIC x3, Federal Reserve, whitepaper), and all 10 bitcoin-vs pages (3-4 sources each from World Gold Council, Bitnodes, SEC, Treasury Dept, RBI, Atlantic Council, BIS, Christie's, Sotheby's, Visa, Federal Reserve, Cambridge Centre for Alt Finance, and Bitcoin whitepaper/source code). CSS: `.sources-section` with subtle styling (14px gray headings, numbered list, muted links that turn orange on hover). i18n: `common_sources_heading` added to all 10 language files (en, de, es, fr, pt, nl, bg, id, th, it) with translated "Sources" heading. Script is idempotent.*
...
- [x] **Consider adding a `llms.txt` file** — This is an emerging standard (similar to robots.txt) that provides instructions to AI crawlers about your site's content, structure, and preferred citation format. Place it at `https://bitcoin.rocks/llms.txt`. ✅ *Completed Mar 29, 2026 — Created `llms.txt` at site root with concise site overview including: organization identity (name, founding date, license, languages, contact), editorial approach, structured content listing of all 30+ educational pages organized by category (Core Educational, Comparison Pages, Business, Community & Outreach) with one-line descriptions and URLs, link to `llms-full.txt` for full content, citation preference format, and key Bitcoin facts summary. ~1,500 words. Also added comment block referencing both files to `robots.txt`.*

- [x] **Consider adding a `llms-full.txt` file** — A companion to `llms.txt` that provides the full Markdown content of your site for AI consumption. This can dramatically improve how AI engines understand and cite your content. ✅ *Completed Mar 29, 2026 — Created `llms-full.txt` at site root containing the full educational content of all pages in clean Markdown format (~18,000 words). Includes: About page, What is Bitcoin? section, Inflation page (with all 15 currency-specific money supply stats), Bank Runs page, Wallet Guide (with 6 wallet recommendations), Buy Bitcoin guide, Lightning Wallet Guide, Compound Inflation Calculator, all 10 Bitcoin-vs comparison pages (each with Markdown tables and detailed explanations), Business section (Why Bitcoin, FAQ with 9 Q&A pairs), Get Involved page, What is Nostr page, and Homepage content overview. Each section includes source URL for attribution. Content assembled from English JSON translation files (single source of truth).*
...
6. **Add "Reviewed for accuracy" badges** (1 hour)
