...
> **Note:** `FRED_API_KEY` and `TWELVEDATA_API_KEY` are read by `forms-backend/inflation-stats.js`, which serves the `/api/inflation-stats?currency=XXX` endpoint that the Next frontend's `<InflationStats>` component fetches on the `/inflation` page. The Next frontend service itself does **not** need these keys — it never calls FRED or TwelveData directly.
